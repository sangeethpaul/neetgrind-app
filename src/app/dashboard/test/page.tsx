...
function TestPageContent() {
  ...
  const subjectMeta: Record<string, { desc: string; color: string; hoverColor: string; bgTint: string; shadow: string }> = {
    biology: {
      desc: "Botany & Zoology mock tests",
    ...
    chemistry: {
      desc: "Organic, Inorganic & Physical chemistry tests",
    ...
    physics: {
      desc: "Mechanics, Electrodynamics & Optics tests",
    ...
    return (
      <div className="space-y-6 w-full max-w-xl mx-auto flex-1 flex flex-col justify-center select-none animate-fade-in-up">
        <div className="text-center space-y-1">
          <p className="text-xs text-muted-foreground font-black tracking-widest uppercase">SMASH MODE</p>
          <h2 className="font-poppins font-black text-3xl text-white tracking-tight">Select Subject</h2>
          <p className="text-xs text-muted-foreground max-w-sm mx-auto leading-relaxed">
            Choose a subject to immediately test your understanding with chapter MCQs.
          </p>
        </div>
        ...
        <div className="space-y-4">
          {subjects.map((sub) => {
            ...
            return (
              <Link key={sub.id} href={`/dashboard/test?subject=${sub.slug}`} className="block group">
                <Card className={cn(
  ...
  if (!currentSubject) {
    return (
      <div className="text-center p-8">
        ...
        <Link href="/dashboard/test" className="text-xs text-blue-400 hover:underline mt-2 inline-block">
          Return to Subjects
        </Link>
      </div>
    );
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredTopics.map((topic) => (
          ...
          >
            ...
            {/* Bottom button bar: SMASH focused */}
