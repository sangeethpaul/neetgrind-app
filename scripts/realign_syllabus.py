...
def realign():
    ...
    # Clean up obsolete topics from database
    curr_slugs = set()
    for subj_slug, classes in CURRICULUM.items():
        for class_num, chapters in classes.items():
            for ch_num in chapters.keys():
                curr_slugs.add(f"{subj_slug}-class-{class_num}-ch-{ch_num}")
                
    db_slugs = set(existing_topics.keys())
    obsolete_slugs = db_slugs - curr_slugs
    if obsolete_slugs:
        print(f"\nCleaning up {len(obsolete_slugs)} obsolete topics from database...")
        for slug in obsolete_slugs:
            try:
                topic_id = existing_topics[slug]
                # Delete test sessions and calendar events to avoid FK constraints violations
                supabase.table("test_sessions").delete().eq("topic_id", topic_id).execute()
                supabase.table("calendar_events").delete().eq("topic_id", topic_id).execute()
                # Delete topic itself (ncert_content and questions cascade delete)
                supabase.table("topics").delete().eq("id", topic_id).execute()
                print(f"      Deleted topic: {slug}")
            except Exception as e:
                print(f"      Failed to delete topic {slug}: {e}")
